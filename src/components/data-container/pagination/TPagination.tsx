...
import type {TPaginationProps, TPaginationRef} from '@/components';
...
const TPagination = forwardRef(
    ({jumperText = '바로가기', onChangePageNumber, ...restProps}: TPaginationProps, ref: Ref<TPaginationRef>) => {
        // region [Hooks]
        ...
        const onClickJumperButton = useCallback(() => {
            ...
            if (validResult === true && onChangePageNumber) {
                onChangePageNumber(jumperPage);
            }
        ...
        return (
            <nav className={`t-pagination ${rootClass}`} style={rootStyle} data-testid={'pagination-root'}>
                <span className={'t-pagination__nav-button-container'}>
                    <TIcon
                        className={`t-pagination__nav-button-container__button ${prevPageButtonClass}`}
                        clickable
                        disabled={props.pageNumber === 1}
                        onClick={onClickPreviousPageSet}
                        onKeyDownEnter={onClickPreviousPageSet}
                        onKeyDownSpace={onClickPreviousPageSet}
                    >
                    ...
                    <TIcon
                        ...
                        onKeyDownSpace={onClickPreviousPage}
                    >
                        keyboard_arrow_left
                    </TIcon>
                </span>
                ...
                <ul className={'t-pagination__page-container'}>
                    {Array(pageRange.max - pageRange.min + 1)
                        .fill(null)
                        .map((_, index) => pageRange.min + index)
                        .map((page) => (
                            <li key={page} className={'t-pagination__page-container__page'}>
                                <button
                                    className={`t-pagination__page-container__page__button ${pageButtonClass(page)}`}
                ...
                <span className={'t-pagination__nav-button-container'}>
                    <TIcon
                        className={`t-pagination__nav-button-container__button ${nextPageButtonClass}`}
                        clickable
                        disabled={props.pageNumber === props.totalPages}
                        onClick={onClickNextPage}
                        onKeyDownEnter={onClickNextPage}
                        onKeyDownSpace={onClickNextPage}
                    >
                    ...
                    <TIcon
                        ...
                        onKeyDownSpace={onClickNextPageSet}
                    >
                ...
                {!props.noJumper && (
                    <div className={'t-pagination__jumper__container'} data-testid={'pagination-jumper-root'}>
                        <TNumberField
                            ref={numberFieldRef}
                            className={'t-pagination__jumper__container__page__field'}
                            value={jumperPage.toString()}
                            min={jumperTextFieldMin}
                            max={props.totalPages}
                            rules={[rule.valueMin(jumperTextFieldMin), rule.valueMax(props.totalPages)]}
                            onChange={onChangeJumperPageNumber}
                            onKeyDownEnter={onClickJumperButton}
                        />
                        <TButton
                            small
                            className={'t-pagination__jumper__container__button'}
                            onClick={onClickJumperButton}
                        >
                            {props.jumperText}
                        </TButton>
                    </div>
