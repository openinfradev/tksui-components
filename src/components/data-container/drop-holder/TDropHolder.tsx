...
import type {CSSProperties, ForwardedRef, MouseEvent, MouseEventHandler, ReactNode} from 'react';
import {forwardRef, useCallback, useImperativeHandle, useMemo, useRef, useState} from 'react';

import useClickOutside from '@/common/hook/UseClickOutside';
import type {DropHolderAlignment, TDropHolderItem, TDropHolderProps, TDropHolderRef} from '@/components';
...
const TDropHolder = forwardRef(
    (
        ...
        const itemClass = useCallback((item: TDropHolderItem): string => {
            ...
            if (item.disabled) {
                clazz.push('t-drop-holder__items__item--disabled');
            }
            if (item.divideSection) {
                clazz.push('t-drop-holder__items__item--divider');
            }
        ...
        const anchorStyle = useMemo((): CSSProperties => {
            if (props.alignment === 'top-left') {
                return {top: `calc(0px - ${props.offset})`, left: '0'};
            }
            if (props.alignment === 'top-center') {
                return {top: `calc(0px - ${props.offset})`, left: '50%'};
            }
            if (props.alignment === 'top-right') {
                return {top: `calc(0px - ${props.offset})`, right: '0'};
            }
            if (props.alignment === 'bottom-left') {
                return {top: `calc(100% + ${props.offset})`, left: '0'};
            }
            if (props.alignment === 'bottom-center') {
                return {top: `calc(100% + ${props.offset})`, left: '50%'};
            }
            if (props.alignment === 'bottom-right') {
                return {top: `calc(100% + ${props.offset})`, right: '0'};
            }
        ...
        const itemsStyle = useMemo((): CSSProperties => {
            if (props.alignment === 'top-left') {
                return {transform: 'translateY(-100%) translateX(0)'};
            }
            if (props.alignment === 'top-center') {
        ...
        const getItemTemplate = useCallback(
            (item: TDropHolderItem): ReactNode => {
        ...
        return (
            <div
                className={`t-drop-holder ${rootClass}`}
                style={rootStyle}
                id={props.id}
                ref={rootRef}
                onClick={onClickRoot}
            ...
            >
                <div className={'t-drop-holder__anchor'} style={anchorStyle} data-testid={'drop-holder-anchor'}>
                    {isOpened &&
                        (!props.customItem ? (
