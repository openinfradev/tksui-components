...
import {TFormColumnProps} from '~/data-container/form/TForm.interface';
...
const TFormColumn = ({
    ...
    const excludedGap = () => {
        ...
        if (props.column === props.span) { return '0px'; }
    ...
    const includedGap = () => {
        ...
        if (props.column === props.span || props.span === 1) { return '0px'; }
    ...
    return (
        <span
            className={`t-form-item ${rootClass()}`}
            style={rootStyle()}
            role={'group'}
            key={props.key}
        >
            ...
            <div className={'t-form-item__content'} style={contentStyle()}>{props.content} </div>
