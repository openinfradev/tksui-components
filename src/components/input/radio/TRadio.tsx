...
import type {TRadioProps} from '@/components';
...
const TRadio = (props: TRadioProps) => {
    ...
    const iconTemplate = useCallback(() => {
        ...
        return (
            <TIcon xsmall className={`t-radio__icon t-radio__icon--${status}`} color={iconColor}>
                {iconType}
            </TIcon>
        );
    ...
    return (
        <div className={`t-radio ${getRootClass()}`} style={getRootStyle()} id={props.id} data-testid={'t-radio-root'}>
            {/* Main */}
            <div
                className={'t-radio__container'}
