...
import type {TDropdownItem, TDropdownProps} from '@/components';
...
const TDropdown = ({
    ...
    const getFilteredItems = useCallback((): TDropdownItem[] => {
        ...
        return props.items.filter((item) => getItemTemplate(item).toLowerCase().includes(filterText?.toLowerCase()));
        ...
        >
            ...
            {!props.noDetail && (
                <div className={'t-dropdown__details'}>
                    <div className={'t-dropdown__details__message'}>{validator.message && `${validator.message}`}</div>
                </div>
            )}
        </div>
    );
