...
describe('TDropHolder', () => {
    ...
    describe('Style', () => {
        it('ClassName Prop applies to root', () => {
            ...
            render(
                <TDropHolder {...baseProps} className={testClass}>
                    button test
                </TDropHolder>
            );
        ...
        it('Style Prop applies to root', () => {
            ...
            render(
                <TDropHolder {...baseProps} style={testStyle}>
                    button test
                </TDropHolder>
            );
            const root = screen.getByTestId('drop-holder-root');
        ...
        it('ID Prop applies to root', () => {
            ...
            render(
                <TDropHolder {...baseProps} id={testId}>
                    button test
                </TDropHolder>
            );
            const root = screen.getByTestId('drop-holder-root');
    ...
    describe('Render', () => {
        ...
        it('When title props is applied, title element will be displayed', async () => {
            ...
            render(
                <TDropHolder {...baseProps} title={title}>
                    Test DropHolder
                </TDropHolder>
            );
    ...
    describe('Item', () => {
        it('When an item is provided with an icon attribute, TIcon should be displayed before the text', async () => {
            ...
            const user = userEvent.setup();
            render(
                <TDropHolder {...baseProps} items={items}>
                    Test DropHolder
                </TDropHolder>
            );
    ...
    describe('Event', () => {
        ...
        it('When the onOpen prop is provided and user clicks root, it will be called exactly once.', async () => {
            ...
            const user = userEvent.setup();
            render(
                <TDropHolder {...baseProps} onOpen={mockOnOpen}>
                    Test DropHolder
                </TDropHolder>
            );
            const root = screen.getByTestId('drop-holder-root');
        ...
        it('When the onClose prop is provided and the user clicks the root element twice, it will be called exactly once.', async () => {
            ...
            const user = userEvent.setup();
            render(
                <TDropHolder {...baseProps} onClose={mockOnClose}>
                    Test DropHolder
                </TDropHolder>
            );
            const root = screen.getByTestId('drop-holder-root');
        ...
        it('When a user clicks the TDropHolder that is provided with customItem, custom item elements are displayed', async () => {
            ...
            render(
                <TDropHolder alignment={'top-center'} customItem={<CustomElement />}>
                    Test DropHolder
                </TDropHolder>
            );
            const root = screen.getByTestId('drop-holder-root');
