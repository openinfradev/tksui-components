...
import {buttonSize, buttonVariant} from '~/button/button/TButton.interface';
...
describe('TButton', () => {
    ...
    describe('size', () => {
        it('When valid size is entered, it will be applied in the classname', () => {
            ...
            // Assert
            expect(buttons[0]).toHaveClass('t-button--xsmall');
            expect(buttons[1]).toHaveClass('t-button--small');
            expect(buttons[2]).toHaveClass('t-button--medium');
            expect(buttons[3]).toHaveClass('t-button--large');
        });
        ...
        it('When invalid size is entered or not entered, medium size will is applied', () => {
            ...
            // Assert
            expect(buttons[0]).toHaveClass('t-button--medium');
            expect(buttons[1]).toHaveClass('t-button--medium');
        });
        ...
        it('When xsmall is applied, button size will be xsmall', () => {
            ...
            // Assert
            expect(button).toHaveClass('t-button--xsmall');
        });
        ...
        it('When small is applied, button size will be small', () => {
            ...
            expect(button).toHaveClass('t-button--small');
        });
        ...
        it('When medium is applied, button size will be medium', () => {
            ...
            // Assert
            expect(button).toHaveClass('t-button--medium');
        });
        ...
        it('When the width prop is used, button width will be it', () => {
            ...
            const widthValue = '50%';
            render(
                <TButton icon={'link'} width={widthValue}>
                    icon test
                </TButton>
            );
            const button = screen.getByRole('button');
            ...
            // Assert
            expect(button).toHaveStyle({width: widthValue, minWidth: widthValue});
        });
    });
    ...
    describe('content', () => {
        it('Children prop is rendered as button content', () => {
            ...
            const button = screen.getByRole('button');
            const buttonContent = within(button).getByText('button test');

            // Assert
            expect(buttonContent).toHaveClass('t-button__content');
        });
        ...
        it('When the icon prop is used, the TIcon is positioned after the content', () => {
            ...
            const button = screen.getByRole('button');
            const iconElement = within(button).getByText('link');

            // Assert
            expect(iconElement).toHaveClass('t-button__content__icon');
        });
    });
    ...
    describe('onClick & disabled', () => {
        it('When button is clicked, onClick event handler will be executed', async () => {
            ...
            // Assert
            let buttonContent = within(button).queryByText('false');
        ...
        it('When disabled prop is applied, clicking the button does not raise an onClick event', async () => {
            ...
            const DisabledTButton = () => {
                const [isClicked, setIsClicked] = useState<boolean>(false);
                return (
                    <TButton disabled onClick={() => setIsClicked(true)}>
                        {isClicked.toString()}
                    </TButton>
                );
            };
            ...
            // Assert
            expect(buttonContent).not.toBeInTheDocument();
        });
    });
    ...
    describe('loading', () => {
        it('When loading prop is applied, loading slices will be rendered instead of content', async () => {
            ...
            // Assert
            expect(button).toHaveClass('t-button--loading');

            // Arrange
            const buttonContent = within(button).queryByText('button test');
            ...
            // Assert
            expect(buttonContentLoading.length).toEqual(3);
        });
    });
});
