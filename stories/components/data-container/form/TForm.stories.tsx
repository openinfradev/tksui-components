    ...
    return (
        <TForm
            ...
            rows={[
                {
                    columns: [
                        {label: 'Artifact URL', required: true, content: <TTextField ref={noRowUrl} rules={rules} value={artifactUrl} onChange={setArtifactUrl} />},
                        {label: 'Port', content: <TTextField ref={noRowPort} rules={rules} counter={20} value={port} onChange={setPort} placeholder={'8080'}/>},
                    ],
                },
                {
                    columns: [
                        {label: 'Profile', content: <TTextField ref={noRowProfile} rules={rules} counter={20} value={profile} onChange={setProfile} />},
                        {label: 'Resource Spec', content: <TDropdown items={resourceSpecItems} value={resourceSpec} onChange={setResourceSpec} />},
                    ],
                },
                {
                    labelVerticalAlign: 'top',
                    columns: [
                        {span: 2, label: 'Description', content: (<TTextField ref={noRowDescriptionRef} rules={rules} multiline counter={100} rows={5} value={description} onChange={setDescription} />)},
                    ],
                },
            ]}
        />
