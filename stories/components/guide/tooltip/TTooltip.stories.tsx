...
const NormalTemplate = (args) => (
    <>
        ...
        <div style={{position: 'absolute', left: 'calc(50% - 86px)', top: 'calc(50% - 24px)'}}>
            <TIcon xlarge clickable tooltipContent={'위로 뜨는 툴팁'} tooltipPlace={'top'} tooltipId={'common-tooltip'}>
                expand_less
            </TIcon>
            <TIcon
                xlarge
            ...
            <br />
            <TButton
                tooltipContent={'tooltipPlace props 에 top, right, bottom, left 중 값을 입력해주세요'}
                tooltipId={'common-tooltip'}
                {...args}
            >
                방향 테스트
...
const CustomTemplate = (args: TTooltipProps) => (
    <>
        <TTooltip id={'custom-html'} {...args}>
            <TIcon style={{marginRight: '8px'}}>fingerprint</TIcon> 지문으로 인증해 주세요. 이런 식으로 툴팁 내부에
            텍스트가 아닌 값을 넣을 수 있어요.
        </TTooltip>

        <TIcon xlarge clickable tooltipId={'custom-html'}>
            fingerprint
        </TIcon>
    </>
);
